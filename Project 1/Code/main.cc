...
void parallel_average(int length);
int importFitnessData(string fileName);
void mergeSort(int left, int right);
void merge(int left, int middle, int right);
double divide_average(int length, int leftBound);
...
//						parallelMergeSort();
						...
						parallel_average(fitnessDataSize);
...
void parallel_average(int length) {
	...
	{
		#pragma omp section
		averageL = divide_average(middleElement, 0); // Recursively split and find average of half #1
		
		#pragma omp section
		averageR = divide_average(length-1, middleElement + 1); // Recursively split and find average of half #2
	}
